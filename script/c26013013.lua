...
function c26013013.initial_effect(c)
	...
	e1:SetCountLimit(1,26013013,EFFECT_COUNT_CODE_OATH)
	e1:SetOperation(c26013013.activate)
	c:RegisterEffect(e1)
	--Frequency
...
function c26013013.filter(c)
	return c:IsSetCard(0x613) and c:IsAbleToGrave()
end
function c26013013.tuner(c,tc)
	return c:IsFaceup() and c:IsCode(tc:GetCode())
end
function c26013013.activate(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	local g=Duel.GetMatchingGroup(c26013013.filter,tp,LOCATION_DECK,0,nil)
	if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(26013013,0)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local sc=g:Select(tp,1,1,nil):GetFirst()
		Duel.SendtoGrave(sc,REASON_EFFECT)
		local tg=Duel.GetMatchingGroup(c26013013.tuner,tp,LOCATION_MZONE,0,nil,sc)
...
function c26013013.cfilter(c,tp)
	return c:IsFaceup(tp) and c:IsSetCard(0x613)
end
function c26013013.refilter(c)
	return c:IsSetCard(0x613) and c:IsMonster() and c:IsAbleToDeckAsCost()
end
function c26013013.recost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c26013013.refilter,tp,LOCATION_GRAVE,0,1,nil) end
...
function c26013013.recon(e,tp,eg,ep,ev,re,r,rp)
	...
	and g and g:IsExists(c26013013.cfilter,1,nil,tp)
